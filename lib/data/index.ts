...
import { courses as fallbackCourses } from './courses'
import { diveSites as fallbackDiveSites } from './dive-sites'
import { faqs as fallbackFaqs } from './faqs'
import {
  adventureOptions as fallbackAdventureOptions,
  experiences as fallbackExperiences,
} from './experiences'
import { galleryItems as fallbackGalleryItems } from './gallery'
import { packages as fallbackPackages } from './packages'
...
async function client(admin = false) {
...
async function withPublicFallback<T>(
  ...
  try {
    ...
    console.warn(`[Fathu Dives] ${label} is unavailable in Supabase; using bundled content.`, error)
    return fallback
  }
}
...
export async function getCourses(admin=false){return withPublicFallback(admin,'Courses',async()=>{const s=await client(admin);let q=s.from('courses').select('*').order('sort_order');if(!admin)q=q.eq('active',true);const {data,error}=await q;fail('Could not load courses',error);return(data??[]).map(mapCourse)},fallbackCourses)}
export const getAdminCourses=()=>getCourses(true)
export async function getCourseBySlug(slug:string,admin=false){return withPublicFallback(admin,'Course',async()=>{const s=await client(admin);let q=s.from('courses').select('*').eq('slug',slug);if(!admin)q=q.eq('active',true);const{data,error}=await q.maybeSingle();fail('Could not load course',error);return data?mapCourse(data as Row):undefined},fallbackCourses.find(x=>x.slug===slug))}
...
export async function getDiveSites(admin=false){return withPublicFallback(admin,'Dive sites',async()=>{const s=await client(admin);let q=s.from('dive_sites').select('*').order('sort_order');if(!admin)q=q.eq('active',true);const{data,error}=await q;fail('Could not load dive sites',error);return(data??[]).map(mapSite)},fallbackDiveSites)}
...
export async function getDiveSiteBySlug(slug:string,admin=false){return withPublicFallback(admin,'Dive site',async()=>{const s=await client(admin);let q=s.from('dive_sites').select('*').eq('slug',slug);if(!admin)q=q.eq('active',true);const{data,error}=await q.maybeSingle();fail('Could not load dive site',error);return data?mapSite(data as Row):undefined},fallbackDiveSites.find(x=>x.slug===slug))}
...
async function gallery(admin=false){return withPublicFallback(admin,'Gallery',async()=>{const s=await client(admin);let q=s.from('gallery_items').select('*').order('sort_order');if(!admin)q=q.eq('active',true);const{data,error}=await q;fail('Could not load gallery',error);return(data??[]).map(mapGallery)},fallbackGalleryItems)}
...
export async function getTestimonials(admin=false){return withPublicFallback(admin,'Testimonials',async()=>{const s=await client(admin);let q=s.from('testimonials').select('*').order('sort_order');if(!admin)q=q.eq('active',true);const{data,error}=await q;fail('Could not load testimonials',error);return(data??[]).map(mapTestimonial)},fallbackTestimonials)}
export const getAdminTestimonials=()=>getTestimonials(true)
async function simple(table:'faqs'|'experiences'|'adventure_options',admin=false){const s=await client(admin);let q=s.from(table).select('*').order('sort_order');if(!admin)q=q.eq('active',true);const{data,error}=await q;fail(`Could not load ${table}`,error);return (data??[]) as Row[]}
...
export async function getFaqs(admin=false){return withPublicFallback(admin,'FAQs',async()=>(await simple('faqs',admin)).map(mapFaq),fallbackFaqs)} export const getAdminFaqs=()=>getFaqs(true)
...
export async function getExperiences(admin=false){return withPublicFallback(admin,'Experiences',async()=>(await simple('experiences',admin)).map(mapExperience),fallbackExperiences)} export const getAdminExperiences=()=>getExperiences(true)
...
export async function getAdventureOptions(admin=false){return withPublicFallback(admin,'Adventure options',async()=>(await simple('adventure_options',admin)).map(mapAdventure),fallbackAdventureOptions)} export const getAdminAdventureOptions=()=>getAdventureOptions(true)
export async function getSiteSettings():Promise<SiteSettings>{const fallback:SiteSettings={id:1,contactEmail:siteConfig.email,whatsapp:siteConfig.whatsapp,phone:siteConfig.phoneDisplay,instagramUrl:siteConfig.socials.instagram,facebookUrl:siteConfig.socials.facebook,youtubeUrl:siteConfig.socials.youtube,tripadvisorUrl:siteConfig.socials.tripadvisor,heroHeadline:siteConfig.tagline,heroSubheadline:siteConfig.description,announcement:'',seoTitle:siteConfig.name,seoDescription:siteConfig.description};return withPublicFallback(false,'Site settings',async()=>{const s=await client();const{data,error}=await s.from('site_settings').select('*').eq('id',1).single();fail('Could not load site settings',error);const r=data as Row;return{id:1,contactEmail:text(r.contact_email),whatsapp:text(r.whatsapp),phone:text(r.phone),instagramUrl:text(r.instagram_url),facebookUrl:text(r.facebook_url),youtubeUrl:text(r.youtube_url),tripadvisorUrl:text(r.tripadvisor_url),heroHeadline:text(r.hero_headline),heroSubheadline:text(r.hero_subheadline),announcement:text(r.announcement),seoTitle:text(r.seo_title),seoDescription:text(r.seo_description)}},fallback)}
