...
export async function getPackages(admin=false) { const s=await client(admin); let q=s.from('packages').select(packageSelect).order('base_price'); if(!admin) q=q.eq('active',true); const {data,error}=await q; fail('Could not load packages',error); return (data??[]).map(mapPackage) }
...
export async function getPackageBySlug(slug:string, admin=false){ const s=await client(admin); let q=s.from('packages').select(packageSelect).eq('slug',slug); if(!admin) q=q.eq('active',true); const {data,error}=await q.maybeSingle(); fail('Could not load package',error); return data ? mapPackage(data as Row) : undefined }
...
export async function getCourses(admin=false){const s=await client(admin);let q=s.from('courses').select('*').order('sort_order');if(!admin)q=q.eq('active',true);const {data,error}=await q;fail('Could not load courses',error);return(data??[]).map(mapCourse)}
export const getAdminCourses=()=>getCourses(true)
export async function getCourseBySlug(slug:string,admin=false){const s=await client(admin);let q=s.from('courses').select('*').eq('slug',slug);if(!admin)q=q.eq('active',true);const{data,error}=await q.maybeSingle();fail('Could not load course',error);return data?mapCourse(data as Row):undefined}
...
export async function getDiveSites(admin=false){const s=await client(admin);let q=s.from('dive_sites').select('*').order('sort_order');if(!admin)q=q.eq('active',true);const{data,error}=await q;fail('Could not load dive sites',error);return(data??[]).map(mapSite)}
...
export async function getDiveSiteBySlug(slug:string,admin=false){const s=await client(admin);let q=s.from('dive_sites').select('*').eq('slug',slug);if(!admin)q=q.eq('active',true);const{data,error}=await q.maybeSingle();fail('Could not load dive site',error);return data?mapSite(data as Row):undefined}
...
async function gallery(admin=false){const s=await client(admin);let q=s.from('gallery_items').select('*').order('sort_order');if(!admin)q=q.eq('active',true);const{data,error}=await q;fail('Could not load gallery',error);return(data??[]).map(mapGallery)}
...
export async function getTestimonials(admin=false){const s=await client(admin);let q=s.from('testimonials').select('*').order('sort_order');if(!admin)q=q.eq('active',true);const{data,error}=await q;fail('Could not load testimonials',error);return(data??[]).map(mapTestimonial)}
